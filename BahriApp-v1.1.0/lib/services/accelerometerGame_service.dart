...
class StepCounter {
  ...
  /// v1.1 (R1-4): the most recent RAW three-axis sample.
  ///
  /// v1.0 had no such field. getOrientation() instead indexed
  /// _accelerationData[0..2] — but that list holds a time series of
  /// MAGNITUDES, so those were the first three magnitude samples of the
  /// session, not x, y and z. The result was exactly one orientation value
  /// per session across all 489 released sessions, clustered at
  /// pitch = -19.8 deg, roll = 37.8 deg, regardless of whether the
  /// participant was walking, jogging or climbing stairs.
  ///
  /// The raw axes were available at the call site all along; they were simply
  /// never retained. They are now.
  List<double>? _lastRawAccel;
  ...
  /// v1.1 (R1-4): the v1.0 cap of 20 steps censored 43.4% of sessions, so
  /// step counts in the released corpus are truncated, not measured. Session
  /// length is now bounded by the task, not by an arbitrary buffer size.
  /// Retained only as a runaway guard, two orders of magnitude higher.
  final int _maxDataEntries = 5000;
  ...
  void _processAccelerometerData(
    ...
    _lastRawAccel = [x, y, z]; // v1.1: retain the axes for getOrientation()
...
// Stop data collection
  ...
  Future<String> _sendDataToDartFrogServer(Map<String, dynamic> data) async {
    final url =
        AppConfig.endpoint('/collect_accelerometer_data');
  ...
  Map<String, double> getOrientation() {
    ...
    // v1.1: the true three axes of the most recent sample. See _lastRawAccel.
    if (_lastRawAccel == null) return {'pitch': 0, 'roll': 0, 'yaw': 0};
    double ax = _lastRawAccel![0];
    double ay = _lastRawAccel![1];
    double az = _lastRawAccel![2];
  ...
  Future<void> initHive() async {
    ...
    await SecureCacheV11.open('offlineAcceloData');
  }
