...
class NetworkManager {
  ...
  Future<void> initHive() async {
    ...
    // v1.1 (R1-6): every offline box is opened with a 256-bit AES cipher whose
    // key lives in the Android Keystore. v1.0 called Hive.openBox() with no
    // encryptionCipher on all eight boxes, so cached behavioural sessions sat
    // on the device as plaintext — the defect behind the withdrawn
    // "end-to-end encryption" claim. SecureCacheV11.initAll() opens the same
    // eight boxes, encrypted.
    await SecureCacheV11.initAll();
  }
  ...
  Future<void> uploadPendingKeystrokeData() async {
    ...
    for (var key in box.keys) {
      ...
      final url =
          AppConfig.endpoint('/collect_keystroke_data');
      try {
  ...
  Future<void> uploadPendingKeystrokeFreeTextData() async {
    ...
    for (var key in box.keys) {
      var requestData = box.get(key);
      final url = AppConfig.endpoint('/collect_keystroke_freetext_data');
      try {
        final response = await http.post(
          url,
          headers: await AuthHeadersV11.json(),
  ...
  Future<void> uploadPendingPasswordFreeTextData() async {
    ...
    for (var key in box.keys) {
      var requestData = box.get(key);
      final url = AppConfig.endpoint('/collect_keystroke_passwordtext_data');
      try {
        final response = await http.post(
          url,
          headers: await AuthHeadersV11.json(),
  ...
  Future<void> uploadPendingTapData() async {
    ...
    for (var key in box.keys) {
      var requestData = box.get(key);
      final url = AppConfig.endpoint('/collect_tap_data');
      try {
        final response = await http.post(
          url,
          headers: await AuthHeadersV11.json(),
  ...
  Future<void> uploadPendingSwipeData() async {
    ...
    for (var key in box.keys) {
      var requestData = box.get(key);
      final url = AppConfig.endpoint('/collect_swipe_data');
      try {
        final response = await http.post(
          url,
          headers: await AuthHeadersV11.json(),
  ...
  Future<void> uploadPendingHandwitingData() async {
    ...
    for (var key in box.keys) {
      var requestData = box.get(key);
      final url =
          AppConfig.endpoint('/collect_Handwriting_data');
      try {
        final response = await http.post(
          url,
          headers: await AuthHeadersV11.json(),
