...
class UserServices {
  ...
  Future<String> registerUserDartFrog(BuildContext context) async {
    ...
    final url = AppConfig.endpoint('/api/v2/register_user'); // Use your Dart Frog server address
    debugPrint("ipgiven");
  ...
  Future<String> loginDartFrog(String username, String password) async {
    ...
    try {
      final response = await http.post(
        AppConfig.endpoint('/api/v2/login'),
  ...
  Future<UsernameCheckResult> checkUsernameAvailability(String username) async {
    ...
    try {
      final response = await http.post(
        AppConfig.endpoint('/api/v2/check_username_availability'),
        headers: <String, String>{
          'Content-Type': 'application/json',
        },
        body: jsonEncode(<String, String>{
