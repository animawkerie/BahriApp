...
class TeamsServices {
  ...
  Future<String> createTeamDartFrog(BuildContext context) async {
    ...
    final url = AppConfig.endpoint('/create_team'); // Use your Dart Frog server address
    // debugPrint("ipgiven");
  ...
  Future<List<Map<String, dynamic>>> getTeams() async {
    try {
      final response = await http.get(AppConfig.endpoint('/get_teams_list')); // Adjust endpoint
  ...
  Future<Map<String, dynamic>> checkUserStatus() async {
    try {
      final response = await http.get(
        AppConfig.endpoint('/check_team_status?uid=$uid'),
      );
  ...
  Future<String> rejectJoinRequest(String teamID, String userID) async {
    try {
      final response = await http.post(
        AppConfig.endpoint('/reject_join_request'), // Adjust to your endpoint
        headers: await AuthHeadersV11.json(),
        body: jsonEncode({
          'teamID': teamID,
          'userID': userID,
  ...
  Future<String> acceptJoinRequest(String teamID, String userID) async {
    try {
      final response = await http.post(
        AppConfig.endpoint('/accept_join_request'), // Adjust to your endpoint
        headers: await AuthHeadersV11.json(),
        body: jsonEncode({
          'teamID': teamID,
          'userID': userID,
  ...
  Future<List<Map<String, dynamic>>> getPendingRequests(String teamID) async {
    try {
      final response = await http.get(
        AppConfig.endpoint('/get_pending_requests?teamID=$teamID'),
      );
  ...
  Future<List<Map<String, dynamic>>> getTeamMembers(String teamID) async {
    try {
      final response = await http.get(
        AppConfig.endpoint('/get_team_members?teamID=$teamID'),
      );
  ...
  Future<String> kickOutMember(String teamID, String userID) async {
    try {
      final response = await http.post(
        AppConfig.endpoint('/kick_out_member'),
        headers: await AuthHeadersV11.json(),
        body: jsonEncode({
          'teamID': teamID,
          'userID': userID,
