...
> **Version 1.1.0.** This release corrects defects found by auditing the v1.0
> acquisition code against the v1.0 released corpus, following peer review of
> the accompanying SoftwareX article. Eighteen exported feature columns are
> retracted, the gyroscope significance filter and orientation estimators are
> replaced, and the security posture is rebuilt — the v1.0 claim of end-to-end
> encryption was inaccurate and is withdrawn in full.
>
> Start with [CHANGELOG.md](CHANGELOG.md), then
> [MIGRATION.md](MIGRATION.md) if you hold v1.0 data,
> [SECURITY.md](SECURITY.md) for the security audit, and
> [docs/data_dictionary.md](docs/data_dictionary.md) for the retracted columns.
...
both apps share the same backend, models, and (where applicable)
...
- 🔒 **Transport and at-rest protection (v1.1)** — HTTPS with certificate
  pinning, an AES-256 encrypted Hive cache keyed from the Android Keystore,
  and bearer-token authentication on every upload. Minimal-demographic
  collection under granular consent.
  **Note:** none of this was present in v1.0. See
  [SECURITY.md](SECURITY.md) before using the v1.0 pilot corpus.
- 📶 **Offline-first sync** — an on-device cache (Hive) queues data locally
  and batch-syncs once connectivity returns, with server-side deduplication
  so a retried upload cannot duplicate a session.
...
3. **Multimodal Acquisition Subsystem** — validates, canonicalizes and
   timestamps each incoming stream on a monotonic clock, attaches per-session
   instrument metadata, and routes it to the Dart Frog ingestion service.
...
Single Flutter Codebase (lib/)
   ...
   │        │
   │        ├── Game Engine ──► Local Hive cache (AES-256, Keystore key)
   │        │                         │
   │        │                         ▼
   │        └── Auth/API Gateway ──► Dart Frog ingestion service (AWS EC2)
   │                   (HTTPS, pinned)        │
   │                                          ▼
   │                                   PostgreSQL
   │                                          │
...
> `lib/features/` (per-modality game logic), `lib/services/` (networking,
> Hive, transport security), `lib/backoffice/` (admin-only screens/widgets),
> `lib/services/v11/` (the corrected v1.1 extractors and security layer).
...
> **If you are using the v1.0 pilot corpus, read the data dictionary first.**
> Eighteen of the 165 exported columns are retracted as invalid, with the
> reason and — where one exists — a corrected definition. Four of them cannot
> be recomputed because the raw inputs were never stored.

### BahriApp requirements

- Android 8.0 (API 26) or higher
- [Flutter SDK](https://flutter.dev/docs/get-started/install) (stable channel)
- A reachable ingestion backend (see [`deploy/`](./deploy) for a local stack
  that needs no author credentials)
...
cp .env.example .env        # then edit; never commit the filled-in .env

# 4. (Optional) bring up a local backend — no author credentials needed
cd deploy && mkdir -p certs && openssl req -x509 -newkey rsa:2048 -nodes \
  -keyout certs/server.key -out certs/server.crt -days 365 -subj "/CN=localhost"
docker compose --env-file ../.env up -d && cd ..

# 5. Run BahriApp on a connected Android device or emulator
flutter run -d <android-device-id> \
  --dart-define=BAHRI_API_BASE=https://10.0.2.2:8443 \
  --dart-define=BAHRI_ALLOW_INSECURE=false
```
...
The endpoint is a build-time define, not a source constant. v1.0 hard-coded
`http://15.184.243.127:8080` in all 24 endpoints, which was both unconfigurable
and unencrypted; `AppConfig.assertValid()` now refuses to start a release build
pointed at a plain-HTTP host.
...
See [`.env.example`](./.env.example) for the full annotated template. The
values that matter:

```
BAHRI_API_BASE=https://api.example.org      # must be https://
BAHRI_CERT_PIN_SHA256=<base64 pin>          # empty = platform trust store
GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json  # dashboard only
```

The Hive cache key is generated on first run and stored in the Android
Keystore. There is no key alias to configure and no key in `.env` — see
[`lib/services/v11/secure_cache_v11.dart`](./lib/services/v11/secure_cache_v11.dart).
...
3. Play; all raw interaction telemetry is timestamped on a monotonic clock,
   cached locally in the encrypted Hive queue, and synced automatically — no
   manual data submission is required.
...
- All gameplay works fully offline. Interaction events are cached locally in
  [Hive](https://pub.dev/packages/hive), encrypted with AES-256 under a key
  held in the Android Keystore (v1.1; v1.0 cached them as plaintext).
- When connectivity is restored, the client batch-syncs queued events to the
  Dart Frog ingestion service over HTTPS.
- Sync is idempotent: each upload carries an `Idempotency-Key` derived from
  `(participantId, sessionId, modality)`, and the server enforces a matching
  unique constraint, so a retry cannot duplicate a session.
- Cached payloads are purged once the server acknowledges them, keeping the
  at-rest window as short as connectivity allows.
...
- Read access to the same backend used by BahriApp, plus a service-account
  JSON for the Firestore-backed export path (set via
...
BahriAdmin reads the same `.env` configuration as BahriApp, plus
...
Each modality is stored as its own table (and, on export, a separate CSV)
with a shared set of identifying fields:
...
See [`docs/data_dictionary.md`](./docs/data_dictionary.md) for the complete
per-modality field reference, **including the eighteen retracted columns**.
...
- Only minimal demographic data is collected (age range and gender), never
  direct identifiers.
- **v1.1** encrypts the on-device cache (AES-256, Keystore-held key) and
  transports uploads over HTTPS with certificate pinning, authenticated with
  a bearer token.
- **v1.0 did none of this.** The earlier claim of end-to-end encryption was
  inaccurate and has been withdrawn. If you are working with the v1.0 pilot
  corpus, read [SECURITY.md](SECURITY.md) for the full audit.
- Administrative exports are role-gated and written to an append-only audit
  table.
...
@article{aseres2026bahriapp,
  ...
  author  = {Animaw Kerie Aseres , Lemlem Kassa Tegegne , and Asrat Mulatu Beyene},
  journal = {SoftwareX},
  year    = {2026}
}
```
